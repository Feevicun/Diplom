...
interface WSMessage {
  type: 'auth' | 'message' | 'typing' | 'read_receipt' | 'user_status' | 'error' | 'chat_list';
  payload: unknown;
}
...
class ChatWebSocketServer {
  ...
  private initialize() {
    this.wss.on('connection', (ws: WebSocketClient, request) => {
      ...
      ws.on('close', () => {
        if (ws.userId) void this.handleUserDisconnect(ws.userId);
      });
  ...
  private async authenticateClient(ws: WebSocketClient, token: string) {
    try {
  ...
  private async handleMessage(ws: WebSocketClient, message: WSMessage) {
    ...
    switch (message.type) {
      case 'message':
        await this.handleChatMessage(ws, message.payload as MessagePayload);
        break;
      case 'typing':
        await this.handleTyping(ws, message.payload as TypingPayload);
        break;
      case 'read_receipt':
        await this.handleReadReceipt(ws, message.payload as ReadReceiptPayload);
        break;
      case 'chat_list':
        await this.sendChatList(ws);
        break;
  ...
  private async handleChatMessage(ws: WebSocketClient, messageData: MessagePayload) {
    if (!ws.userId) {
    ...
    try {
      ...
      const messageResult = await pool.query(
        ...
        [chatId, ws.userId, content, type, replyTo, attachment ? JSON.stringify(attachment) : null]
      );
  ...
  private async handleTyping(ws: WebSocketClient, typingData: TypingPayload) {
    if (!ws.userId) return;
  ...
  private async handleReadReceipt(ws: WebSocketClient, receiptData: ReadReceiptPayload) {
    if (!ws.userId) return;
  ...
  private async sendChatList(ws: WebSocketClient) {
    if (!ws.userId) return;
    try {
  ...
  private async getUserChats(userId: string): Promise<ChatUser[]> {
    ...
    return Promise.all(result.rows.map(async (row) => {
      ...
      if (row.type === 'group') {
        const membersResult = await pool.query(
          `SELECT u.id, u.name, u.email, u.role as type, u.avatar_url, u.is_online
           FROM chat_members cm
        ...
        members = membersResult.rows.map((memberRow: MemberRow) => ({
          id: memberRow.id.toString(),
          name: memberRow.name,
          avatar: memberRow.name.split(' ').map((n: string) => n[0]).join('').toUpperCase(),
      ...
      return {
        ...
        type: row.type as 'student' | 'supervisor' | 'group' | 'admin',
  ...
  private async broadcastToChat(chatId: string, message: WSMessage, excludeUserId?: string) {
    try {
      interface ChatMemberRow { user_id: string; }
      const membersResult = await pool.query(
        'SELECT user_id FROM chat_members WHERE chat_id = $1',
        [chatId]
      );

      const memberIds = membersResult.rows.map((row: ChatMemberRow) => row.user_id.toString());
  ...
  private async updateChatListForParticipants(chatId: string) {
    try {
      interface ChatMemberRow { user_id: string; }
      const membersResult = await pool.query(
        'SELECT user_id FROM chat_members WHERE chat_id = $1',
