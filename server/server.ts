...
// --- GET: Отримати всі чати для користувача ---
app.get("/api/chats", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const chats = result.rows.map(chat => ({
      ...
      unreadCount: 0, // можна додати логіку підрахунку непрочитаних
    }));
...
app.post("/api/chats", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const chatResult = await pool.query(
      `INSERT INTO chats (name, type, description, created_at, updated_at)
    ...
    // Додаємо учасників (включно з автором)
...
// --- GET: Отримати повідомлення певного чату ---
app.get("/api/chats/:chatId/messages", authenticateToken, async (req: Request, res: Response) => {
  try {
...
// --- POST: Додати повідомлення до чату ---
app.post("/api/chats/:chatId/messages", authenticateToken, async (req: Request, res: Response) => {
  try {
    const { chatId } = req.params;
    ...
    const result = await pool.query(
      `INSERT INTO messages (chat_id, sender, content, timestamp, attachment)
       VALUES ($1, $2, $3, NOW(), $4)
       RETURNING *`,
      [chatId, senderId, content, attachment]
    );
    ...
    res.status(500).json({ message: "Database error creating message" });
  }
});
