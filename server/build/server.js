...
app.put("/api/update-profile", async (req, res) => {
    try {
       ...
       WHERE id = $4
       RETURNING id, name, email, role, avatar_url`, [name, email, avatarUrl, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "User not found" });
        }
...
app.post("/api/events", authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO events ("userEmail", title, date, type) VALUES ($1, $2, $3, $4) RETURNING *`, [userEmail, title, date, type]);
...
app.get("/api/events", authenticateToken, async (req, res) => {
    try {
        ...
        if (!userEmail)
            return res.status(401).json({ message: "Unauthorized" });
        const result = await pool.query(`SELECT id, title, date, type FROM events WHERE "userEmail" = $1 ORDER BY date`, [userEmail]);
        res.json(result.rows);
    }
    catch (error) {
...
app.put("/api/resources/:id", authenticateToken, async (req, res) => {
    try {
       ...
       WHERE id = $5
       RETURNING *`, [title, description, link, category, id]);
        res.json({
